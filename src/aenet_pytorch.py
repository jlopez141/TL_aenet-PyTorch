...
tin.device = device
if tin.pytorch_seed: torch.manual_seed(tin.pytorch_seed)
if tin.numpy_seed:   np.random.seed(tin.numpy_seed)
if tin.verbose: io_input_reading(tin)


# If transfer, we need to read stp parameters from the pretrained model
if tin.mode == "transfer": init_transfer_stp(tin)
