...
def read_keyword_freeze_layers(lines):
	value = None
	found = False
	for iline in range(len(lines)):
		read_keyword = lines[iline].split()[0].lower()

		if read_keyword == "freeze_layers":
			found = True
			value = lines[iline].split()[1:]

	value = np.array( [ x=="1" or x==True or x.lower()=="true" or x.lower()=="t" for x in value])
...
def read_transfer_networks(lines, sys_species):
	value = None
	found = False
	for iline in range(len(lines)):
		read_keyword = lines[iline].split()[0].lower()

		if read_keyword == "transfer_networks":
			found = True
			networks_line = iline+1
	...
	transfer_param          = {}
	transfer_param["names"] = names

	return transfer_param
...
def read_train_in(infile):
	with open(infile, "r") as f:
		...
		E_params     , found = read_keyword_argument_next_line("e_normalization", lines)
		if found: tin.E_min         = float(E_params["e_min"])
		if found: tin.E_max         = float(E_params["e_max"])
		if found: tin.E_avg         = float(E_params["e_avg"])
		if found: tin.predef_E_norm = True

		tin.initialize()

		# If transfer learning
		if tin.mode == "transfer":

			transfer_model, _ = read_keyword_argument_same_line("transfer_model", lines)
			transfer_param    = read_transfer_networks(lines, tin.sys_species)
			freeze_layers     = read_keyword_freeze_layers(lines)

			transfer_param["transfer_model"] = transfer_model
			transfer_param["freeze_layers"]  = freeze_layers

			tin.transfer_param = transfer_param
